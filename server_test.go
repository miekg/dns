...
// HelloServerNXDomainThenSuccess first sends an NXDomain response (simulating
// DNS hijacking), then sends the real success response.
func HelloServerNXDomainThenSuccess(w ResponseWriter, req *Msg) {
	// First response: NXDomain (potentially hijacked)
	m := new(Msg)
	m.SetReply(req)
	m.Rcode = RcodeNameError // NXDomain
	w.WriteMsg(m)

	// Second response: Success (real response)
