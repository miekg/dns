...
// Issue #312.
func TestClientSetUDPSize(t *testing.T) {
	c := &Client{SetUDPSize: true}
	type Test struct {
		bufsize  uint16
		expected uint16
	}
	...
	for i, tc := range tests {
		...
		case 0:
			if o := m.IsEdns0(); o != nil {
				t.Fatalf("%d: OPT RR should not have been added", i)
			}
		default:
			o := m.IsEdns0()
			if o == nil {
				t.Fatalf("%d: OPT RR should have been added", i)
			}
			if o.UDPSize() != tc.expected {
				t.Fatalf("%d: bufsize: expected %d, got %d", i, tc.expected, o.UDPSize())
			}
		}
	}
}
