...
func main() {
	...
	// Generate compressedLen()
	fmt.Fprint(b, "// compressedLen() functions\n")
	for _, name := range namedTypes {
		if _, ok := skipLen[name]; ok {
			continue
		}
		...
		fmt.Fprintf(b, "func (rr *%s) compressedLen(off int, compression map[string]struct{}, compress bool) int {\n", name)
		fmt.Fprintf(b, "l := rr.Hdr.compressedLen(off, compression, compress)\n")
		for i := 1; i < st.NumFields(); i++ {
			o := func(s string) { fmt.Fprintf(b, s, st.Field(i).Name()) }
